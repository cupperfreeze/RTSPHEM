%analog to 'evaluateInterface', but contains reference data for example
%presented in [2] Section 4.1

function [interfaceLength, coordTripel,varargout] = evaluateInterfaceReference(grid, Xi, distFunctions, slt, varargin)
...
%If yes, calculate alpha as in (21) and calculate approximate coordinates 
%of interface point
...
x = pointsOnInterface(identifyInterface==1,1);
y = pointsOnInterface(identifyInterface==1,2);
[x,key] = sort(x);
y=y(key);
f=@(out) max(abs(interp1(x,y,out)),0);
error(1) =  integral(f,-0.5,-0.25);

x = pointsOnInterface(identifyInterface==2,1);
y = pointsOnInterface(identifyInterface==2,2);
[x,ia,ic]=unique(x);
y=y(ia);
[x,key] = sort(x);
y=y(key);
f=@(out) max(abs(interp1(x,y,out) - 0.25),0);
error(2) =  integral(f,0,0.5);
f=@(out) max(abs(interp1(x,y,out,'linear') - sqrt(0.25^2-out.^2)),0);
error(2) = error(2) + integral(f,-0.25,0);

x = pointsOnInterface(identifyInterface==3,1);
y = pointsOnInterface(identifyInterface==3,2);
idx = x<-10^(-10);
x=x(idx);
y=y(idx);
[x,ia,ic]=unique(x);
...
[y,key] = sort(y);
x=x(key);
f=@(out) max(abs(interp1(y,x,out)),0);
error(3) = error(3) +  integral(f,-0.5,0);
varargout{1}=error;
end
...
if ~slt
    ...
    out = double(Xi);
    %out(Xi==1) = 0; out(Xi==2) = -0.5; out(Xi==3) = -1;

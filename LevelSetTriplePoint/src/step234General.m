%Same as 'step234' for arbitrary number of phases

function [dV, S] = step234General(distFunctions, grid, Xi, interfaceVelocities, restrictDist)
...
for i = 1:length(Initials)
    ...
    dVInitial(InitialIndex) = min(abs(dMinValue(InitialIndex) - distFunctions(InitialIndex, mod((dMinIndex(InitialIndex)+(0:(numberOfPhases - 2))), numberOfPhases) + 1))) / 2;

    [sortedDist, localInd] = min(abs(dMinValue(InitialIndex) - distFunctions(InitialIndex, mod((dMinIndex(InitialIndex)+(0:(numberOfPhases - 2))), numberOfPhases) + 1)));
    closeDist = find((sortedDist-sortedDist(1, 1) < 10*eps));

    if numel(closeDist) > 1
        initialS(InitialIndex) = sign(initialS(InitialIndex)) * ...
            min(abs(interfaceVelocities(dMinIndex(InitialIndex), mod((dMinIndex(InitialIndex)+localInd(closeDist)), numberOfPhases) + 1)));
    end
...
for i = 1:length(exceptions)
    ...
    if sum(isnan(neighbors)) == 0 %spare, in proximity of outer boundary
        CrossInterfaceNeighbors = neighbors(Xi(exceptions(i)) ~= Xi(neighbors));
        [~, index] = max(dVInitial(CrossInterfaceNeighbors)-1*eps*abs(initialS(CrossInterfaceNeighbors))');

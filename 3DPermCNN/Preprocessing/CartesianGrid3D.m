classdef CartesianGrid3D < handle
    ...
    methods
        ...
        function this = CartesianGrid3D( dim, bounds, numParts )
            assert( numel(dim) < 2 && numel(dim) > 0 );
            assert( abs( dim - round(dim) ) < eps );
            assert( dim > 0 && dim < 4, [ 'Dimension not supported. ', ...
...
%             end
          ...
          %speedup in dim=2
              if ~isa(this, 'FoldedCartesianGrid') 

                local = cell(2,2,dim);                                            %direction f/b, depth, dim
                for i=1:dim
                    local{1,1,i} = this.getForward(1:this.nodes,i);
                    local{2,1,i} = this.getBackward(1:this.nodes,i);
                    local{1,2,i} = this.getRelative(1:this.nodes, dim*(1:this.dimension == i));
                    local{2,2,i} = this.getRelative(1:this.nodes, -dim*(1:this.dimension == i));
                end
                this.pull = local;
        ...
        function retIndex = getRelative( this, index, shift )
            ...
            subscCell = mat2cell( subsc( isInterior, : ) , ...
             ...
             retIndex(isInterior) =subsc(isInterior,1);
             for i=2:this.dimension 
                 retIndex(isInterior) =   retIndex(isInterior)+prod(this.nodesPerDimension(1:(i-1))) * ( subsc(isInterior,i) - 1 );
            end

...
trainingMincut = [porosities(InidzesOfData), areas(InidzesOfData)./10000];
...
validationMincut = trainingMincut(ceil(PercentageOfTrainImages*numberOfData): end,:);
...
trainingMincut = trainingMincut(1:ceil(PercentageOfTrainImages*numberOfData-1),:);
...
additionalLayers = [image3dInputLayer([1,2,1,1],'Name','physics'); fullyConnectedLayer(64,'Name','physics2','WeightsInitializer','ones','Biasinitializer','zero')];
...
a2=arrayDatastore(trainingMincut,"IterationDimension",1);
...
a2=arrayDatastore(validationMincut,"IterationDimension",1);
...
for i=1:size(validationIm,5)
    ...
    a2=arrayDatastore(validationMincut(i,:),"IterationDimension",1);
...
for i=1:size(trainingIm,5)
    ...
    a2=arrayDatastore(trainingMincut(i,:),"IterationDimension",1);
...
save('CNNPhysPoroSurf.mat','net','predicted','validationData', 'predictedTrain', 'trainingData','validationMincut','trainingMincut','-v7.3');

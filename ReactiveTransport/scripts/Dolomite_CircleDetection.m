% Perform micro-macro simulation with dolomite grains. In this script,
% a-priori checking for circular geometries within the unit cells is implemented
...
numPartitionsMicroscale = 50; % Number of partitions in each direction
macroscaleStepSize = lengthXAxis / 70;
(27 * 4); % [ dm ]
meanGrainSize = 10^(-4); % [ dm ]

% Set parameters if not defined globally
if (numel(numPartitionsMicroscale) == 0)
    numPartitionsMicroscale = 50; % Number of partitions in each direction
end

if (numel(macroscaleStepSize) == 0)
    macroscaleStepSize = lengthXAxis / 70;
    (27 * 4); % [ dm ]
end
...
% Physical parameters
dimension = 2;
spaceScaleFactor = 50; %10*sqrt(13*27); % spaceScaleFactor [length of Y] = 1 [cm]
...
for (i = 1:numberOfSlices) %, numProc)
    tic
    % check weather cell geometry resembles circle
...
while transportStepper.next
    ...
    % check weather geometry resembles circle; if yes, use lookup table
    for (i = 1:numberOfSlices) %, numProc)
        ...
        if isBalli
            ...
            if transportStepper.curtau * normalSpeedMax(i) > 0.2 / numPartitionsMicroscale
                tic
                SOL = solveSystemFE(microscaleGrid, cellProblemSystemMatrix, rhs, isDoF);

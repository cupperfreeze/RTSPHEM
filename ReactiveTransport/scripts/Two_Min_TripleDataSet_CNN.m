...
%Interpolate geometry evolution grid to diffusion and CNN grid
...
toc
CNNLevel(:, :, 1, (numberOfSlices + 1):2*numberOfSlices) = turn(CNNLevel (:, :, 1, 1:numberOfSlices), -pi/2);
CNN = exp(10*(net.predict(CNNLevel, 'ExecutionEnvironment', CNNHardware) - 0.9)) * (meanGrainSize^2);
...
while transportStepper.next
    ...
    %use CNN for permeability prediction
    if numel(index1) > 0
        help7(:, :, 1, (numel(index1) + 1):2*numel(index1)) = turn(help7(:, :, 1, 1:numel(index1)), -pi/2);
        CNN = exp(10*(net.predict(help7, 'ExecutionEnvironment', CNNHardware) - 0.9));

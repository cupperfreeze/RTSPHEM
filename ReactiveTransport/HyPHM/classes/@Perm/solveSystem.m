%> @file Perm/solveSystem.m Solve the linear system for the Stokes problem.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%> @retval  U        Scalar @f$x@f$-velocity of the current time @f$[ \#V+\#E\times 1 ]@f$ for force term e_1.
%> @retval  V        Scalar @f$y@f$-velocity of the current time @f$[ \#V+\#E\times 1 ]@f$ for force term e_1.
%> @retval  U2       Scalar @f$x@f$-velocity of the current time @f$[ \#V+\#E\times 1 ]@f$ for force term e_2.
%> @retval  V2       Scalar @f$y@f$-velocity of the current time @f$[ \#V+\#E\times 1 ]@f$ for force term e_2.
...
tAssembly = tic;
ActiveTriangles = find(~markDirV(g.V0T(:, 1)) | ~markDirV(g.V0T(:, 2)) | ~markDirV(g.V0T(:, 3)));
...
% Z  = zeros(1, numVE);
% bigA = [ dataN*this.globA ,                      sparse(numVE,numVE),           -this.globB',              Z';
%         sparse(numVE,numVE),                     dataN*this.globA,              -this.globC',              Z';
...
U = X(1:numVE, 1);
V = X(numVE+1:2*numVE, 1);
P = X(2*numVE+1:2*numVE+numV, 1); % there is the scalar mass balance dummy at the end of X
%printline(3,  '...done [%.3f sec]', toc(tSolving)) % stop timer solving
U2 = X(1:numVE, 2);
V2 = X(numVE+1:2*numVE, 2);

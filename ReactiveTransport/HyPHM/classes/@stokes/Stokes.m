%> @file Stokes.m Class for the Stokes problem.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%>
...
%>   <b>(ii) Unknowns</b>
%>
%>     - Stokes.U velocity @f$\vec{u}@f$ (type @c P2P2)
%>     - Stokes.P pressure @f$p@f$       (type @c P1)
%>
%>
%>   <b>(iii) Coefficient Functions</b>
%>
%>     - Stokes.N  dynamic viscosity @f$\mu@f$                 (type @c C)
%>     - Stokes.F  external force field  @f$\vec{f}@f$         (type @c P2P2)
%>     - Stokes.uD Dirichlet boundary @f$\vec{u}_\mathrm{D}@f$ (type @c P2P2, sampled at boundary only)
%>
%>
%>   <b>(iv) Flags</b>
%>
...
%>   <b>(iii) Solving</b>
%>
%>     - Stokes.computeLevel(Variable for @f$\vec{u}@f$, Variable for @f$p@f$)
%>
%>

classdef Stokes < AbstractProblem % < hgsetget & handle
    ...
    methods
        ...
        function this = Stokes(grid, stepper, name)
            msg = 'HyPHM: The constructor for this class (now) is Stokes(Grid, Stepper, name).';
            assert(nargin == 3, msg)
    ...
    methods
        ...
        function this = set.isEvolution(this, bool)
            assert(islogical(bool), 'HyPHM: Stokes.isEvolution expects true or false.')

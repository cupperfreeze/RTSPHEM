%> @file Transport.m Class for the transport problem.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%>
%> An instance of this class should contain all data that are required by
%> Transport.computeLevelr(), i.e., coefficient functions, initial, and boundary data,
...
%> Transport is generated (see also Transport.initVoidCoeffs).  You can
%> either define new instances of Variable and mount those to the problem
%> (i.e. to an instance of Transport) or modify them directly:
%>
%> @code
%> % st = Stepper(...);
%> % g  = Grid(...);
%> t = Transport(s, st, 'my transport problem');
...
%> @endcode
%>
%> In the latter example, Transport.C.name is still <code>'C'</code> while
%> in the first one, Transport.C.name is <code>'convection'</code>.
%>
...
%> <code>Transport.isUpwind</code>, which is <code>'none'</code> by default:
...
classdef Transport < AbstractProblem
    ...
    methods
        ...
        function this = Transport(grid, stepper, name)
            msg = 'HyPHM: The constructor for this class (now) is Transport(Grid, Stepper, name).';
            assert(nargin == 3, msg)

%> @file Transport/solveSystem.m Solves the linear system of the Transport problem

function [Qnew, Unew, bigA, bigB] = solveSystem(d, curTau, B, C, D, E, bQ, bU, markNeumE, markFreeE, dataC, dataE, Y, isSlt, isVisPattern)
...
idx1 = logical([markFreeE; ones(g.numT, 1); mUidx]); % mUidx = false if there's no mass balance
idx2 = logical([markFreeE; ones(g.numT, 1)]);
X = [Y; zeros(g.numT, 1)];

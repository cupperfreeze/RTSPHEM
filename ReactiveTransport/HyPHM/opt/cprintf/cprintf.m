...
if nargin < 1, showDemo(majorVersion, minorVersion);
    return;
end
...
% In compiled mode
try useDesktop = usejava('desktop');
catch, useDesktop = false;
end
...
for elementIdx = 1:numElements
    ...
    links = docElement.getAttribute('LinkStartTokens');
    urls = docElement.getAttribute('HtmlLink');
    try bolds = docElement.getAttribute('BoldStartTokens');
    catch, bolds = [];
    end
    txt = {};

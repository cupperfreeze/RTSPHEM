function [globalStiffness, rightHandSide, isDegreeOfFreedom, ...
    ...
    assembleCellProblem(grid, levelSet, varargin)
% Assemble FE system for 2D cell problems.
...
for tri = 1:numTriangles
    ...
    %         stiffnessLocalToGlobalIndex = sub2ind( grid.nodes * ones(1,2), x, y );
    %         matrixEntries( stiffnessLocalToGlobalIndex(:) ) = ...
    %             matrixEntries( stiffnessLocalToGlobalIndex(:) ) ...
    %             + triVolume * localStiffness(:);
...
if length(varargin) == 0
    ...
    clear triangleSurfaces
    triangleSurfaces{1} = zeros(numTriangles, 1);
    triangleSurfaces{2} = zeros(numTriangles, 1);
    triangleSurfaces{1}(varargin{1} == 1) = interfaceLengths(varargin{1} == 1);
    triangleSurfaces{2}(varargin{1} == 2) = interfaceLengths(varargin{1} == 2);
end

rowColIndex(indexCounter:end, :) = [];
matrixEntries(indexCounter:end) = [];

%     zeroIndex = find( abs( matrixEntries ) < EPS );
%     rowColIndex( zeroIndex, : ) = [];
%     matrixEntries( zeroIndex ) = [];
...
globalStiffness = sparse(rowColIndex(:, 1), rowColIndex(:, 2), ...
    matrixEntries, numNodes, numNodes);
%     globalStiffness = reshape( matrixEntries, grid.nodes, grid.nodes );
